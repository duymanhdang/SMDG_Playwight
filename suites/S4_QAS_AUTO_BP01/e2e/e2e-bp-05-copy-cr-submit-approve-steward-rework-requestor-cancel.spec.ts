...
import { RequestorActions, createRequestorActions } from '../../../pages/actions/RequestorActions';

/**
 * E2E-TC05: Copy CR → Approve → Steward Rework → Requestor Cancel
 ...
 *   [Steward]   Activation → Assign → Rework → verify REWORK
 *   [Requestor] My Request → Cancel CR → verify CANCELLED
 *
 * Multi-context: 3 browser contexts — session isolation per role
 */

const TD = getTestData('e2e-bp-05');

test.describe(
  'E2E-BP-05: Copy Request → Approve → Steward Rework → Requestor Cancel',
  { tag: ['@cr', '@bp', '@bp:bp-cr-lifecycle'] },
  () => {
    test(
      'Full flow: Copy CR → Approve → Steward Rework → Requestor Cancel',
      ...
      async ({ browser }) => {
        ...
        const requestorActions = createRequestorActions(requestorPage);
        ...
        try {
          ...
          console.log('\n══════════════════════════════════════');
          console.log('  PHASE 3: STEWARD — Assign & Rework CR');
          console.log('══════════════════════════════════════');
          ...
          await steward.rework(newCR, 'Steward rework this CR to requestor !');
          ...
          await requestorActions.cancelFromDetail(newCR);
          await myRequest.waitForStatus(newCR, TD.expectedStatuses.afterCancel);
          await myRequest.verifyStatus(newCR, TD.expectedStatuses.afterCancel);

          console.log(`\n✅ PHASE 4 COMPLETE — CR ${newCR} → ${TD.expectedStatuses.afterCancel}`);

          console.log('\n══════════════════════════════════════');
          console.log(`✅ E2E-TC05 PASSED`);
          console.log(`   CR: ${newCR}`);
          console.log(`   Flow: COPY CR → SUBMITTED → APPROVED → REWORK → CANCELLED`);
          console.log('══════════════════════════════════════');
        } finally {
